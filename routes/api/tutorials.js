...
router.get("/all", (req, res) => {
  ...
  Tutorial.find({})
    ...
    .catch((err) =>
      res.status(500).json({ error: "Unable to get tutorials", errorMsg: err })
    );
});
  ...
  Tutorial.find({ tags: { $regex: tag, $options: "i" } })
    ...
    .then((tutorials) => {
      if (tutorials.length === 0)
        return res.json({ tutorials, error: "No tutorials found" });
...
router.get("/:tutorial", (req, res) => {
  ...
  Tutorial.findById(req.params.tutorial)
    ...
    .catch((err) =>
      res.status(500).json({ error: "Unable to get tutorial", errorMsg: err })
    );
});
  ...
  (req, res) => {
    ...
    Tutorial.find({ submittedBy: req.user._id })
      ...
      .catch((err) =>
        res
          .status(500)
          .json({ error: "Unable to get tutorials", errorMsg: err })
      );
  }
);
  ...
  (req, res) => {
    ...
    User.findById(req.user._id)
      ...
      .catch((err) =>
        res
          .status(500)
          .json({ error: "Unable to get favorites", errorMsg: err })
      );
  }
);
      ...
      .then((tutorial) => {
        User.findByIdAndUpdate(
          ...
          .catch((err) =>
            res
              .status(500)
              .json({ error: "Unable to update user data", errorMsg: err })
          );
      })
      .catch((err) =>
        res
          .status(500)
          .json({ error: "Unable to save tutorial", errorMsg: err })
      );
  }
);
...
router.post(
  ...
  (req, res) => {
    ...
    Tutorial.findByIdAndUpdate(
      ...
      .catch((err) =>
        res.status(500).json({ error: "Unable to post review", errorMsg: err })
      );
  }
);
  ...
  (req, res) => {
    ...
    User.findByIdAndUpdate(
      ...
      .catch((err) =>
        res
          .status(500)
          .json({ error: "Unable to add to favorites", errorMsg: err })
      );
  }
);
...
router.post(
  ...
  (req, res) => {
    ...
    Tutorial.findByIdAndUpdate(
      ...
      User.findByIdAndUpdate(
        req.user._id,
        { $addToSet: { upvotes: tutorial._id } },
        { new: true }
      )
        .then((user) => res.json({ msg: "Upvote Added" }))
        .catch((err) =>
          res.status(500).json({ error: "Unable to upvote", errorMsg: err })
        );
    });
  }
);
  ...
  (req, res) => {
    ...
    User.findByIdAndUpdate(
      ...
      .catch((err) =>
        res
          .status(500)
          .json({ error: "Unable to remove from favorites", errorMsg: err })
      );
  }
);
    ...
    Tutorial.findByIdAndUpdate(
      ...
      User.findByIdAndUpdate(
        ...
        .catch((err) =>
          res
            .status(500)
            .json({ error: "Unable to remove upvote", errorMsg: err })
        );
    });
  }
);
